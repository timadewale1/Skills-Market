...
export async function POST(request: NextRequest) {
  try {
          ...
          <div style="padding: 40px 30px;">
            <div style="background: linear-gradient(135deg, #fed7aa 0%, #fdba74 50%, #f97316 100%); border-radius: 8px; padding: 3px; margin-bottom: 30px;">
              <div style="background: #ffffff; border-radius: 6px; padding: 25px;">
                ...
                <p style="color: #475569; margin: 0; font-size: 16px; line-height: 1.6;">
                  <strong>${talentName}</strong> has requested a payout for the gig <strong>"${gigTitle}"</strong>.
                </p>
                ${amount ? `<p style="color: #475569; margin: 15px 0 0 0; font-size: 16px; line-height: 1.6;">Amount: <strong>₦${Number(amount).toLocaleString()}</strong></p>` : ''}
                <p style="color: #ea580c; margin: 15px 0 0 0; font-size: 14px; font-weight: 600;">
                  ⏰ Client has 24 hours to review and approve.
                </p>
              </div>
            </div>

            <div style="text-align: center; margin: 30px 0;">
              <a href="https://skills-market.vercel.app/admin/workspaces/${workspaceId}" style="display: inline-block; padding: 14px 28px; background: linear-gradient(135deg, #f97316 0%, #ea580c 100%); color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px; box-shadow: 0 4px 15px rgba(249, 115, 22, 0.4);">
                📋 Review Payout
              </a>
            </div>
            ...
            <div style="background: #fef3c7; border-left: 4px solid #f97316; padding: 20px; border-radius: 8px; margin: 20px 0;">
              <p style="margin: 0; color: #92400e; font-size: 14px;">
                💡 <strong>Note:</strong> Monitor payout requests to ensure project quality standards are met before release.
              </p>
            </div>
          </div>
    ...
    await notifyAdmins({
      ...
      message: `${talentName} requested payout for "${gigTitle}"${amount ? ` (₦${Number(amount).toLocaleString()})` : ""}`,
